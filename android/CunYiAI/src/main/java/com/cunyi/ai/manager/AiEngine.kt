...
/**
 * AI聊天引擎（RAG增强版）
 * 医疗问题：先查本地知识库 → 把结果注入 prompt → AI 综合组织回答
 * 非医疗问题：纯闲聊规则
 */
object AiEngine {
    ...
    /**
     * 带知识库上下文的回复生成（RAG 模式）
     * medicalContext: 从 MedicalKBQA 查询到的相关医疗数据
     */
    fun generateResponse(userMessage: String, medicalContext: MedicalContext? = null): String {
        val msg = userMessage.trim()
        if (msg.isBlank()) return "请说点什么吧～"

        // 有医疗知识上下文 → AI 综合回答
        if (medicalContext != null && medicalContext.hasData) {
            return generateRAGResponse(msg, medicalContext)
        }

        // 无上下文，走纯规则
        return generateRuleBasedResponse(msg)
    }
    ...
    // ===== RAG 模式：知识库上下文 + AI 组织语言 =====
    private fun generateRAGResponse(userMessage: String, ctx: MedicalContext): String {
        return buildString {
            append("根据医疗知识库，我找到了以下信息：\n\n")

            if (ctx.symptom.isNotEmpty()) {
                append("【症状】\n${ctx.symptom}\n\n")
            }
            if (ctx.cause.isNotEmpty()) {
                append("【病因】\n${ctx.cause}\n\n")
            }
            if (ctx.cureWay.isNotEmpty()) {
                append("【治疗方法】\n${ctx.cureWay}\n\n")
            }
            if (ctx.prevent.isNotEmpty()) {
                append("【预防措施】\n${ctx.prevent}\n\n")
            }
            if (ctx.food.isNotEmpty()) {
                append("【饮食建议】\n${ctx.food}\n\n")
            }
            if (ctx.drug.isNotEmpty()) {
                append("【常用药物】\n${ctx.drug}\n\n")
            }
            if (ctx.check.isNotEmpty()) {
                append("【检查项目】\n${ctx.check}\n\n")
            }
            if (ctx.department.isNotEmpty()) {
                append("【就诊科室】${ctx.department}\n\n")
            }
            if (ctx.other.isNotEmpty()) {
                append("【其他信息】\n${ctx.other}\n\n")
            }

            append("⚠️ 以上仅供参考，不能替代医生诊断。如有不适，请及时就医。")
        }
    }

    // ===== 纯规则回复 =====
    private fun generateRuleBasedResponse(msg: String): String {
        // 问候
    ...
    fun isModelReady(): Boolean {
        return try {
            ModelManager.getStaticInstance()?.hasAnyModel() == true
        } catch (e: Exception) {
            false
        }
    }

    fun getModelStatus(): String = if (isModelReady()) "AI已就绪" else "请先下载AI模型"
